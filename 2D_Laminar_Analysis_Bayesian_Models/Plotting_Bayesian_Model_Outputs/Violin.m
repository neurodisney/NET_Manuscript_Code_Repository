classdef Violin < handle
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            quartiles = quantile(data, [0.025, 0.25, 0.5, 0.75, 0.975]);
            ...
            switch args.QuartileStyle
                case 'shadow'
                    ...
                    w(h>quartiles(5))=flat((h>quartiles(5)));
                ...
                case 'boxplot'
                    obj.BoxPlot = ... % plot color will be overwritten later
                        ...
                        [quartiles(1) quartiles(1) quartiles(5) quartiles(5)], ...
                        [1 1 1]);
                case 'none'
            end
            ...
            IQR = quartiles(5) - quartiles(1);
            %lowhisker = quartiles(1) - 1.5*IQR;
            lowhisker = quartiles(1);
            %lowhisker = max(lowhisker, min(data(data > lowhisker)));
            %hiwhisker = quartiles(5) + 1.5*IQR;
            hiwhisker = quartiles(5);
           ...
           % Median
          %obj.MedianPlot = scatter(pos, quartiles(3), args.MedianMarkerSize, [2 2 2], 'filled');
...
%                 [], [1 1 1], 'filled', 'v');
            obj.NotchPlots = ...
                scatter(pos, quartiles(1)-1.57*IQR/sqrt(length(data)), ...
                [], [1 1 1], 'filled', '^');
            obj.NotchPlots(2) = ...
                scatter(pos, quartiles(5)+1.57*IQR/sqrt(length(data)), ...
                [], [1 1 1], 'filled', 'v');
        ...
        function set.ViolinColor(obj, color)
            ...
            obj.MeanPlot.Color = [0.25 0.25 0.25];
            if ~isempty(obj.ViolinPlot2)
    ...
    methods (Access=private)
        function results = checkInputs(~, data, pos, varargin)
            ...
            p.addParameter('ShowNotches', true, isscalarlogical);
            p.addParameter('ShowMean', true, isscalarlogical);
            p.addParameter('ShowBox', true, isscalarlogical);
            p.addParameter('ShowMedian', false, isscalarlogical);
